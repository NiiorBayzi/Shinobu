...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription(`Envie a primeira palavra para começar o termo.`)
    ...
    message.reply({ embeds: [embed] }).then((msg) => {
      ...
      collector.on("collect", (m) => {
        ...
        answer = parseText(m.content.trim()).split(/ +/g);
        if (answer.length < 4) return message.reply(`**(${emoji.error}) | ${parseText(m.author.username)}**, coloque no mínimo **5 letras**.`).then((a) => { setTimeout(() => a.delete(), 2500) });
        if (!termo.info(message.author.id)) {
          created = termo.create(answer[0].toLowerCase(), m.author.id);
        } else {
          ...
          created = termo.add(answer[0].toLowerCase(), m.author.id);
        }
