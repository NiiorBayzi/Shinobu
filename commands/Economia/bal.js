module.exports = {
  ...
  run: async (client, message, args, db) => {
    ...
    let pos = db.filter((a) => a.cash > 0).sort((a, b) => b.cash - a.cash).findIndex(x => x._id === message.author.id) + 1;

    message.reply(`**(${emoji.wallet}) | <@${message.author.id}>**, **${parseText(user.username)}** possui **${emoji.starcoins} ${data.cash}** eu sua carteira${pos > 0 ? `.\n> **(${emoji.rank}) |** Sabia que **${parseText(message.author.username)}** está na posição **#${pos}** no ranking?` : "."}`);
  }
}
