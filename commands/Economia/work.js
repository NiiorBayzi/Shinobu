module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if ((db.get(`users/${message.author.id}/cooldown/work`) || 0) > Date.now()) return message.reply(`<:_:986467656058171422> | Você precisa esperar **\`${parseTime(db.get(`users.${message.author.id}.cds.work`))}\`** para trabalhar novamente.`);
